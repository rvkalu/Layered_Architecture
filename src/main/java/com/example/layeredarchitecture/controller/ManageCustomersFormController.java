...
import com.example.layeredarchitecture.tdm.CustomerTM;
...
public class ManageCustomersFormController {
    ...
    private void loadAllCustomers() {
        ...
        try {
            ...
            ArrayList<CustomerDTO> rst = customerDAO.getAllCustomer();


            for (CustomerDTO x : rst){

                tblCustomers.getItems().add(new CustomerTM(x.getId(), x.getName(), x.getAddress()));
    ...
    private String generateNewId() {
        try {Connection connection = DBConnection.getDbConnection().getConnection();
            ResultSet rst = connection.createStatement().executeQuery("SELECT id FROM Customer ORDER BY id DESC LIMIT 1;");if (rst.next()) {
